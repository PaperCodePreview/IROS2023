...
device_0 = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")
...
def build_dataset(num_robots):
    ...
    train_env = dataset_class(
        ...
        view_db_file=feat_db_file_train, split='train', name='train', batch_size=6
    )
    ...
    val_seen_env = dataset_class(
        ...
        os.path.join(dataset_dir, 'connectivity'), image_feat_size=image_feat_size, angle_feat_size=angle_feat_size,
        num_robots=num_robots,
        view_db_file=feat_db_file_train, start_vp_cache='val_seen_start_vps.json', split='dev', name='val_seen', batch_size=6
    )
...
def main():
    ...
    # listner.load('checkpoints/Vanilla-VLNBERT_latest')
