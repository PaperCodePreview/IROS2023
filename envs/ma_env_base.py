...
from task.multi_point_nav import MultiPointNavTask
...
class BaseEnv(gym.Env):
    ...
    def load(self):
        ...
        if self.config['scene'] == 'empty':
            ...
            self.simulator.import_scene(
                scene,
                load_texture=self.config.get('load_texture', True) and self.config['scene_id'] not in ['CVPR2022',
                                                                                                       'Cross'])
...
class MAGibsonEnv(BaseEnv):
    ...
    def load_miscellaneous_variables(self):
        ...
        """
        self.current_step = np.zeros(self.num_robots)
    ...
    def get_equi(self, robot_id):
        return self.simulator.renderer.get_equi()
    ...
    def get_state(self, collision_links=[]):
        ...
        if 'adj_mat' in self.output:
            # Empty adjacency matrix - an identity matrix
            adj_mat = np.eye(self.num_robots, dtype=np.bool)

            for i, robot in enumerate(self.robots):
                for j in range(i+1, self.num_robots):
                    if self.finish is not None and (self.finish[j] or self.finish[i]):
                        continue
                    # tmp = l2_distance(robot.get_position()[:2], self.robots[j].get_position()[:2])
                    if l2_distance(robot.get_position()[:2], self.robots[j].get_position()[:2]) <= self.config['con_threshold']:
                        adj_mat[i][j] = True
                        adj_mat[j][i] = True

            state['adj_mat'] = adj_mat
    ...
    def run_simulation(self):
        ...
        """
        self.simulator_step()
        collision_links = [list(p.getContactPoints(bodyA=self.robots[na].robot_ids[0])) for na in
    ...
    def step(self, union_actions):
        ...
        actions = union_actions['actions']
        new_targets = union_actions['new_targets']

        for target in new_targets:
            self.current_step[target['robot_idx']] = 0
            self.collision_step[target['robot_idx']] = 0
            self.collision_links[target['robot_idx']] = []
            self.task.reset_target(self, target['robot_idx'], target['target'])
        ...
        self.collision_step += [int(item) for item in map(len, collision_links)]
        ...
        if self.task is not None:
                ...
                self, collision_links, actions, info)
    ...
    def check_robots_collision(self, robot_index, pos):
        has_collision = False
        for i in range(len(self.robots)):
            if i == robot_index:
                continue
            if l2_distance(self.robots[i].get_position(), pos) < self.config.get("dist_tol", 0.5):
                has_collision = True
    ...
    def base_point_mask(self, x, y, shape):
        if self.base_mask is None or shape[0] != self.base_mask.shape[0] or shape[1] != self.base_mask.shape[1]:
            ...
            self.base_mask = (skimage.morphology.binary_dilation(mask, skimage.morphology.disk(
                self.config['reset_max_dist'])) ^ skimage.morphology.binary_dilation(mask, skimage.morphology.disk(
                self.config['reset_min_dist']))) > 0
...
class iGibson(Gym):
    ...
    def __init__(self, config_file, horizon=None, gamma=0.99, is_discrete=False,
                 width=None, height=None, debug_gui=False, task_random=False, num_robots=1):
        """
        Constructor.
        ...
        config['task_random'] = task_random
        ...
        # env = iGibsonEnv(config_file=config, action_timestep=1 / 10.0, physics_timestep=1 / 120.0, mode='gui' if debug_gui else 'headless')
        env = MAGibsonEnv(config_file=config, mode='gui' if debug_gui else 'headless')

        # env = iGibsonWrapper(env)

        self.env = env

        self._img_size = env.observation_space['rgb'].shape[0:2]
    ...
    def reset_robot_position(self, robot_index):
        self.env.task.reset_robot_position(self.env, robot_index)

    def reset(self, pos=None):
        if pos is not None:
            self.env.task.reset_position(pos[0], pos[1])

        state = self.env.reset()
        if 'adj_mat' in state:
            state['adj_mat'] = np.eye(self.env.num_robots, dtype=np.bool)
        return state
    ...
    def step(self, action):
        ...
        self._image = self._convert_observation(obs['rgb'][0].copy())
        return obs, reward, absorbing, info

    def remove_robot(self, robot_index):
        self.env.robots[robot_index].set_position([100.0 * (robot_index + 1), 100.0 * (robot_index + 1), 100.0 * (robot_index + 1)])
