...
from navigation.algorithm_v2.vlnbert.graph_utils import GraphMap
from navigation.algorithm_v2.utils.ops import pad_tensors_wgrad
from models import VLNBERTDUET, Critic


class GMapObjectNavAgent(Seq2SeqAgent):

    def __init__(self, train_env, planner_device, args, controller_device=None, controller_params=None,
                 policy_params=None,
        ...
        # Models
        self.vln_bert = VLNBERTDUET(self.args).to(planner_device)
        self.critic = Critic().to(planner_device)
    ...
    def _panorama_feature_variable(self, obs):
        ...
        for i, ob in enumerate(obs):
            ...
            for j, cc in enumerate(ob['candidate']):
                # if cc['pointId'] not in used_viewidxs:
    ...
    def _teacher_action(self, obs, vpids, ended, in_progress, visited_masks=None):
        """
    ...
    def make_equiv_action(self, a_t, gmaps, obs, language_feature, traj=None, use_controller=False):
        """
        ...
        for i, scan_ob in enumerate(obs):
            for j, ob in enumerate(scan_ob):
                if self.episode_len[cnt] <= self.episode_max_len:
                    action = a_t[cnt]
                    if action is not None and action != -2:  # None is the <stop> action
                        ...
                        if use_controller:
                            target = self.env.get_node_position(ob['scan'], action)
                            new_targets.append({'robot_idx': j, 'target': target})

                        view_points.append(action)
        ...
        if use_controller:
            for i in range(self.env.num_robots):
                if a_t[0][i] is None or a_t[0][i] == -3:
                    self.mdp.remove_robot(i)

        return a_t, in_progress
    ...
    # @profile
    def rollout(self, train_ml=None, train_rl=False, reset=True, use_controller=False, controller_fit_steps=None,
                val=False):
        if use_controller:
        ...
        while True:
            for i, gmap in enumerate(gmaps):
                if not ended[i] and not in_progress[i]:
            ...
            if train_ml is not None:
                ...
                nav_targets = self._teacher_action(
                    obs, nav_vpids, ended, in_progress,
                ...
                if self.args.fusion in ['avg', 'dynamic'] and self.args.loss_nav_3:
                    ...
                    local_nav_targets = self._teacher_action(
                        obs, nav_inputs['vp_cand_vpids'], ended, in_progress, visited_masks=None
                    )
            ...
            cpu_a_t = []
            for i in range(batch_size):
                if a_t_stop[i] or ended[i] or nav_inputs['no_vp_left'][i] or in_progress[i]:
                    if in_progress[i]:
                        cpu_a_t.append(-2)
                    else:
                        cpu_a_t.append(None)
            ...
            self.make_equiv_action(cpu_a_t, gmaps, obs.reshape([num_scans, num_robots]), language_inputs, traj, use_controller)
            for i in range(batch_size):
                if (not ended[i]) and just_ended[i] and not in_progress[i]:
            ...
            ended[:] = np.logical_or(ended, (np.logical_or([x is None for x in cpu_a_t], cut)))
            cuts.append(cut)
    ...
    def train(self, feedback='teacher', use_controller=False, controller_fit_steps=1024, **kwargs):
        ...
        while True:
            ...
            if self.args.train_alg == 'imitation':
                ...
                self.rollout(
                    train_ml=1., train_rl=False, use_controller=use_controller,
                ...
                if self.args.ml_weight != 0:
                    self.feedback = 'teacher'
                    self.rollout(
                        train_ml=self.args.teacher_weight, train_rl=False, use_controller=use_controller,
                        controller_fit_steps=controller_fit_steps, **kwargs
                    )
                self.feedback = self.args.dagger_sample
                self.rollout(train_ml=1., train_rl=False, use_controller=use_controller,
                    controller_fit_steps=controller_fit_steps, **kwargs)
            else:
                if self.args.ml_weight != 0:
                    self.feedback = 'teacher'
    ...
    def test(self, val_env=None, use_dropout=False, use_controller=False, feedback='argmax'):
        self.feedback = feedback
        if use_dropout:
            self.vln_bert.train()
            self.critic.train()
        ...
        while True:
            for traj in self.rollout(use_controller=use_controller, val=True):
                self.loss = 0
