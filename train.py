...
# from navigation.algorithm.envs.ma_env_base import iGibson
from planner import Seq2SeqAgent
from planner_duet import GMapObjectNavAgent
from envs.env import PlannerEnv
from param import args

from mushroom_rl.core import Logger

# dataset_dir = '/home/xyz9911/PycharmProjects/igibson/datasets/igibson_matterport_v2'
dataset_dir = '/root/IROS2023/datasets/igibson_matterport_v2'

device_1 = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")
device_0 = torch.device("cuda:2" if torch.cuda.is_available() else "cpu")
...
def build_dataset(num_robots):
    ...
    if args.model == 'Vanilla-VLNBERT':
        ...
        feat_db_file_train = os.path.join(dataset_dir, 'train', 'features_vit_base_patch16_224')
        feat_db_file_unseen = os.path.join(dataset_dir, 'val_unseen', 'features_vit_base_patch16_224')
        feat_db_file_test = os.path.join(dataset_dir, 'test', 'features_vit_base_patch16_224')
        image_feat_size = 768
        angle_feat_size = 4
    ...
    train_env = dataset_class(
        ...
        view_db_file=feat_db_file_train, split='train', name='train', batch_size=2
    )
    ...
    val_seen_env = dataset_class(
        ...
        os.path.join(dataset_dir, 'connectivity'), image_feat_size=image_feat_size, angle_feat_size=angle_feat_size,
        num_robots=num_robots,
        view_db_file=feat_db_file_train, start_vp_cache='val_seen_start_vps.json', split='dev', name='val_seen', batch_size=2
    )
    ...
    val_unseen_env = dataset_class(
        ...
        os.path.join(dataset_dir, 'connectivity'), image_feat_size=image_feat_size, angle_feat_size=angle_feat_size,
        num_robots=num_robots,
        view_db_file=feat_db_file_unseen, start_vp_cache='val_unseen_start_vps.json', split='dev', name='val_unseen',
        batch_size=2
    )
...
def train(listner, train_env, val_env_seen, val_env_unseen, igibson_env=None, n_steps_per_fit=None, eval_first=True):
    ...
    for idx in range(start_iter, start_iter + args.iters, 1):
        ...
        if env_name in best_val:
            if score_summary['sr'] >= best_val[env_name]['sr']:
                ...
                listner.save(idx, os.path.join(args.ckpt_dir, "%s_step_%i_memory_%i_robots_%i_best_%s" % (args.model, idx, args.use_memory, listner.env.num_robots, env_name)))
...
def main():
    num_robots = 1
    # args.model = 'DUET'
    # train_env_0, val_env_0 = build_dataset(num_robots)
    ...
    # listner = agent_class(train_env, device_0, device_1, igibson_env=None, controller_params=None,
    #                       policy_params=None, logger=logger)
    listner = agent_class(train_env, device_0, args)
    listner.load('/data/checkpoints/VLNBERT4seen')
    # listner.load_controller('SingleGaussianTorchPolicy.pt')

    # train(train_env, igibson_env=mdp, n_steps_per_fit=n_steps_per_fit, alg_params=alg_params, policy_params=policy_params)
    train(listner, train_env, val_seen_env, val_unseen_env, igibson_env=None, n_steps_per_fit=None)
