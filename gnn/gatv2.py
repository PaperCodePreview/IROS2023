...
class GraphAttentionV2Layer(nn.Module):
    ...
    def forward(self, h: torch.Tensor, adj_mat: torch.Tensor):
        ...
        * `adj_mat` is the adjacency matrix of shape `[n_nodes, n_nodes, n_heads]`.
        ...
        g_l = self.linear_l(h).view(shape[:-1] + (self.n_heads, self.n_hidden))
        g_r = self.linear_r(h).view(shape[:-1] + (self.n_heads, self.n_hidden))
        ...
        prefix = (1,) if len(shape[:-2]) > 0 else tuple()
        g_l_repeat = g_l.repeat(prefix + (n_nodes, 1, 1))
        ...
        # where each node embedding is repeated `n_nodes` times.
        g_r_repeat_interleave = g_r.repeat_interleave(n_nodes, dim=-3)
        ...
        g_sum = g_sum.view(shape[:-2] + (n_nodes, n_nodes, self.n_heads, self.n_hidden))
        ...
        assert adj_mat.shape[-3] == 1 or adj_mat.shape[-3] == n_nodes
        assert adj_mat.shape[-2] == 1 or adj_mat.shape[-2] == n_nodes
        assert adj_mat.shape[-1] == 1 or adj_mat.shape[-1] == self.n_heads
        ...
        if self.is_concat:
            ...
            return attn_res.reshape(shape[:-2] + (n_nodes, self.n_heads * self.n_hidden))
